...
-   **Multiline closures need explicit parameter** — `each { |it| $it.name }` not `each { .name }`.
...
1.  `nu-lint --fix script.nu` — 150+ lint rules with auto-fix (idioms, POSIX patterns, dead code, type safety)
2.  `nufmt script.nu` — official AST-based formatter
3.  `nu --ide-check 10 script.nu` — parse error diagnostics (built into nu)
